...
((("Lightning encrypted transport protocol","Noise XK")))((("Noise Protocol Framework","Noise XK")))((("Noise_XK")))噪声协议非常灵活，因为它通告了几个
握手，每次握手都有不同的安全和隐私属性
可供选择的协议实施者。深入探索每一个
握手及其各种权衡超出了本章的范围。
话虽如此，闪电网络使用一种特定的握手方式，称为
`噪音_XK`。 ((("identity hiding")))这次握手提供的唯一属性是__identity hiding__：为了让一个节点发起与另一个节点的连接，它
必须首先知道它的公钥。从机械上讲，这意味着公钥
响应者的信息实际上从未在
握手。相反，一系列巧妙的椭圆曲线 Diffie–Hellman (ECDH) 和
消息验证代码 (MAC) 检查用于验证
响应者。
...
((("handshake","notation and protocol flow")))((("Lightning encrypted transport protocol","handshake notation and protocol flow")))((("Noise_XK","handshake notation and protocol flow")))每次握手通常由几个步骤组成。每一步都有一些
（可能）加密材料被发送给对方，ECDH（或
几个）被执行，握手的结果被“混合”成
协议_抄本_。该成绩单用于验证
协议并有助于阻止中间人攻击。在
握手结束时，生成两个密钥，`ck` 和 `k`，用于
在整个生命周期内加密消息（`k`）和轮换密钥（`ck`）
会议。
...
在握手的上下文中，s 通常是一个长期静态公钥。在我们的例子中，使用的公钥加密系统是一个椭圆曲线系统，用 secp256k1 曲线实例化，它在比特币的其他地方使用。在整个握手过程中会生成几个临时密钥。我们使用 e 来指代一个新的临时密钥。两个密钥之间的 ECDH 操作表示为两个密钥的串联。例如，ee 表示两个临时密钥之间的 ECDH 操作。
...
该协议从响应者静态密钥的“预传输”开始
(`rs`) 给发起者。在执行握手之前，发起者要
生成自己的静态密钥 (`s`)。在握手的每一步中，所有
通过网络发送的材料和发送/使用的密钥是递增的
散列成_handshake digest_，`h`。这个摘要永远不会通过
握手期间连线，而是用作“关联数据”时
AEAD（经过验证的关联数据加密）通过网络发送。
((("AD (associated data)")))((("associated data (AD)")))_Associated data_ (AD) 允许加密协议验证附加信息
信息以及密文数据包。在其他域中，AD 可能是
域名或数据包的纯文本部分。
...
`h` 的存在确保如果传输的握手的一部分
消息被替换，那么对方会注意到。每一步，MAC
检查摘要。如果 MAC 检查成功，则接收方知道
到那时握手已经成功。否则，如果 MAC
检查失败，则握手过程失败，并且连接
应该终止。
...
该协议还为每个握手消息添加了一条新数据：一个协议
版本。初始协议版本为“0”。在撰写本文时，没有新的
协议版本已创建。结果，如果对等方收到一个版本
不是“0”，那么他们应该拒绝握手发起尝试。
...
就密码原语而言，SHA-256 用作哈希函数
选择，`secp256k1` 作为椭圆曲线，`ChaChaPoly-130` 作为 AEAD
（对称加密）构造。
...
噪声协议的每个变体都有一个唯一的 ASCII 字符串用于引用它。确保两方使用相同的协议
变体，ASCII 字符串被散列成一个摘要，用于初始化
起始握手状态。在闪电网络的背景下，ASCII
描述协议的字符串是“Noise_XK_secp256k1_ChaChaPoly_SHA256”。
...
((("Lightning encrypted transport protocol","handshake in three acts", id="ix_14_encrypted_transport-asciidoc2", range="startofrange")))((("Noise_XK","handshake in three acts", id="ix_14_encrypted_transport-asciidoc3", range="startofrange")))握手部分可以分为三个不同的“行为”。
整个握手在发起者和响应者之间需要 1.5 次往返。
在每个动作中，双方都会发送一条消息。握手
消息是以协议版本为前缀的固定大小的有效负载。
...
`ck`:: _chaining key_。这个值是所有的累加哈希值
   以前的 ECDH 输出。在握手结束时，使用`ck`导出
   Lightning 消息的加密密钥。
...
`h`:: _handshake hash_。这个值是_all_的累加散列
...
鉴于此握手和消息传递会话状态，我们将定义一系列
将对握手和消息传递状态进行操作的函数。什么时候
描述握手协议，我们将以某种方式使用这些变量
类似于伪代码以减少解释的冗长
协议中的每一步。我们将定义的_functional_原语
握手为：
